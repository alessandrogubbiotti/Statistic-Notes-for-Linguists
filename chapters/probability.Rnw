...
\label{s:probability}
	...
	The relations we require are two, see Definition~\ref{d:prob}: the certain event has probability $1$, and the probability of the "or" of two mutually exclusive events is the sum of their probabilities.\sidenote{In mathematical jargon, we require additivity and not $\sigma$-additivity.} This assumption will not affect the exposition since we will deal mostly with exaamples where sample space is finite, that is, where the algebra of events $\mf F$ is finite.  

In these notes we mostly consider algebras $\mathfrak F$ whose sample space $\Omega$ is finite. In this case a probability is determined by the values $\mathbb P(\omega)$ it assigns to the elementary events $\omega \in \Omega$, see Theorem~\ref{t:finite}, and one can skip directly to Section~\ref{ss:finite}. 

	For some of the sample spaces we are interested in, however, we sometimes need to perform (countably) infinite intersections or union of events. Think about Exercise~\ref{exercise:game_2}, where the event $G$ = "The game never ends" is the "and" of the infinitely many events "The experiment is not completed before $n$ rolls", $n \in \mathbb N$, that is, $G = \bigcap_{n = 1}^\infty E_n^c$, and to Model~\ref{ex:unif}, where the event "$X = a$" is the "and" of the events "$X \in [a - \frac1n, a + \frac1n]$", $n \in \mathbb N$.
In this case it is useful to assume that the algebra of events is a $\sigma$-algebra (that is, is closed under countable union of events) and that the probability satisfies the \emph{Kolmogorov axioms}, where assumption ? is replaced by the stronger condition of $\sigma$-additivity.
 
We will not treat the Kolmogorov axioms rigorously and we will point out when we are requiring them in the cases in which we use them: when computing the probabilities of some particular events of Model~\ref{ex:infinite_coin}, when dealing with Bernoulli trials, see Definition~\ref{d:Bernoulli_measure}, and of Model~\ref{ex:unif}, when dealing with continuous random variables, see Definition~\ref{d:continuous}.
