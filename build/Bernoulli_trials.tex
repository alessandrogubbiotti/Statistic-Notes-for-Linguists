...
\begin{alltt}
\hldef{n} \hlkwb{<-} \hlnum{1000} \hlcom{#number of variables to simulate}
\hldef{a} \hlkwb{<-} \hlnum{0.7} \hlcom{#probability of jumping to 1 while at 0}
\hldef{b} \hlkwb{<-} \hlnum{0.2} \hlcom{#probability of jumping to 0 while at 1}
...
\hldef{\}}
\hldef{X} \hlkwb{<-} \hlkwd{simulate_Markov_chain}\hldef{(n,a,b)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in simulate\_Markov\_chain(n, a, b): non trovo la funzione "{}simulate\_Markov\_chain"{}}}\begin{alltt}
\hldef{X[}\hlnum{1}\hlopt{:}\hlnum{30}\hldef{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: oggetto 'X' non trovato}}\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{ger} \hlkwb{<-} \hldef{rdracor}\hlopt{::}\hlkwd{get_dracor}\hldef{(}\hlkwc{corpus} \hldef{=} \hlsng{'ger'}\hldef{)}
\hldef{binary_transform_ger} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{drama_name}\hldef{)\{}
  \hldef{text} \hlkwb{<-} \hldef{rdracor}\hlopt{::}\hlkwd{get_text_chr_spoken}\hldef{(}\hlkwc{play} \hldef{= drama_name,} \hlkwc{corpus} \hldef{=} \hlsng{"ger"}\hldef{)}
  \hldef{all_text} \hlkwb{<-} \hlkwd{paste}\hldef{(text,} \hlkwc{collapse} \hldef{=} \hlsng{""}\hldef{)}
  \hldef{letters_vector} \hlkwb{<-} \hlkwd{tolower}\hldef{(}\hlkwd{unlist}\hldef{(}\hlkwd{strsplit}\hldef{(all_text,} \hlsng{""}\hldef{)))}
  \hldef{spoken_chars} \hlkwb{<-} \hlkwd{c}\hldef{(letters,}\hlsng{" ̈a"}\hldef{,} \hlsng{" ̈o"}\hldef{,} \hlsng{" ̈u"}\hldef{,} \hlsng{"ß"}\hldef{)}
  \hldef{filtered_letters_vector} \hlkwb{<-} \hldef{letters_vector[ letters_vector} \hlopt{%in%} \hldef{spoken_chars]} \hlcom{# Keep only german_vowels <- c("a", "e", "i", "o", "u", " ̈a", " ̈o", " ̈u")}
  \hldef{binary_sequence} \hlkwb{<-} \hlkwd{ifelse}\hldef{(filtered_letters_vector} \hlopt{%in%} \hldef{german_vowels,} \hlnum{0}\hldef{,} \hlnum{1}\hldef{)}
  \hlkwd{return}\hldef{(binary_sequence)}
\hldef{\}}
\hldef{text_01} \hlkwb{<-} \hlkwd{vector}\hldef{(}\hlkwc{mode} \hldef{=} \hlsng{'numeric'}\hldef{)}
\hlkwa{for}\hldef{( play_name} \hlkwa{in} \hldef{ger}\hlopt{$}\hldef{playName[}\hlnum{1}\hlopt{:}\hlnum{10}\hldef{])\{}
 \hldef{text_01} \hlkwb{<-} \hlkwd{c}\hldef{(vec,} \hlkwd{binary_transform_ger}\hldef{(play_name))}
\hldef{\}}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: oggetto 'vec' non trovato}}\begin{alltt}
\hldef{vec[}\hlnum{1}\hlopt{:}\hlnum{30}\hldef{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: oggetto 'vec' non trovato}}\end{kframe}
\end{knitrout}
	\end{example}
	\begin{ExerciseList}
